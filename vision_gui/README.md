...
6. Copy the Larix WebRTC URL into Larix Broadcaster on the iPhone.
7. Use Test WebRTC Receiver to confirm the DesktopGUI receiver is listening.
8. Save Settings.
...
The DesktopGUI starts a local WebRTC WHIP receiver for the iPhone side-view camera. In Larix Broadcaster, set the connection URL to the Larix WebRTC URL shown on Setup, for example `http://MAIN_LINUX_PC_IP:8899/whip/side`.
...
7. Side-View Table Z Calibration: open the monitor board full-screen on the MacBook, start Larix Broadcaster on the iPhone using the DesktopGUI WebRTC URL, then set the table line and save safe claw-height samples. The GUI never lowers the arm automatically.
